...
from keyboards import (
    ...
    friend_players_keyboard,
    friend_rounds_keyboard,
...
class TruthOrDareBot:
    ...
    def _format_categories(self, categories: list[str] | None) -> str:
        if not categories:
            return "Любые"
        labels = [self._category_labels.get(cat, cat) for cat in categories]
        return ", ".join(labels)

    @staticmethod
    def format_age_label_static(min_age: int | None, max_age: int | None) -> str:
        if min_age is None and max_age is None:
            return "🎂 Возраст: Любой"
        if min_age is not None and max_age is not None:
            return f"🎂 Возраст: {min_age}-{max_age}"
        if min_age is not None:
            return f"🎂 Возраст: от {min_age}"
        return f"🎂 Возраст: до {max_age}"

    def _format_age_range(self, min_age: int | None, max_age: int | None) -> str:
        if min_age is None and max_age is None:
            return "Любой"
        if min_age is not None and max_age is not None:
    ...
    def _friend_room_text(self, game_state) -> str:
        cats_text = self._format_categories(game_state.categories)
        return (
            "👥 Комната создана!\n\n"
            f"Код приглашения: <code>{game_state.invite_code}</code>\n\n"
            f"Категории: {cats_text}\n"
            f"Раундов в игре: {game_state.max_rounds}\n"
            f"Игроков может быть: {game_state.max_players}\n"
            "Когда все соберутся — настрой категории, раунды и лимит игроков кнопками ниже и жми \"Начать игру\"."
        )

    def _in_active_game(self, user_id: int):
        game = self.game_logic.get_game_for_user(user_id)
        if not game:
            return False
        return bool(game.started)
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if context.user_data.get("awaiting_search_age_input"):
            ...
            if numbers:
                ...
                await update.message.reply_text(
                    ...
                    reply_markup=search_preferences_keyboard(
                        user_data.get("search_gender", "Любой"),
                        self.format_age_label_static(min_age, max_age),
                    ),
                    parse_mode="HTML",
                )
                return
        ...
        if chat.type != "private":
            ...
            return
        if self._in_active_game(user.id) and text in {
            "🎮 Найти игру",
            "👥 С друзьями",
            "📊 Статистика",
            "⭐ Премиум",
            ...
            await update.message.reply_text(
                "Ты сейчас в игре. Дождись окончания или заверши её кнопкой в сообщении игры."
            )
            return
        if text == "🎮 Найти игру":
            msg = await update.message.reply_text(
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not data.startswith("join_") and not data.startswith("continue_"):
        ...
        if data == "cancel_search":
            await self.cancel_random_search(query)
            return
        if data.startswith("friend_cats_"):
            await self.edit_friend_categories(query, context, data)
            return
        if data.startswith("friend_rounds_"):
            await self.prompt_friend_rounds(query, context, data)
            return
        if data.startswith("friend_round_set_"):
            await self.set_friend_rounds(query, data)
            return
        if data.startswith("friend_players_") and not data.startswith("friend_players_set_"):
            await self.prompt_friend_players(query, data)
            return
        if data.startswith("friend_players_set_"):
            await self.set_friend_players(query, data)
            return
        if data.startswith("friend_back_"):
            await self.show_friend_room_panel(query, data)
            return
        ...
        if data == "set_age":
            ...
            return
        if data == "pref_age_edit":
            await self.prompt_search_age_input(query, context)
            return
        if data.startswith("continue_"):
            await self.continue_turn(query, context, data)
            return
    ...
    async def start_random_game(self, query, context):
        ...
        if not db.can_use_random_search(telegram_id):
            ...
            return
        categories_raw = user_data.get("categories")
        categories = None
        if categories_raw:
            try:
                categories = eval(categories_raw)
                ...
                categories = None
        # случайный поиск не учитывает параметры пола/возраста
        search_gender = None
        search_age_min = None
        search_age_max = None
        gender = user_data.get("gender")
        age = user_data.get("age")
        is_premium = bool(int(user_data.get("is_premium") or 0))
        ...
        if game_state is None:
            ...
            msg = await query.edit_message_text(
                ...
                "Фильтры пола/возраста: не учитываются в случайном поиске.\n\n"
    ...
    async def cancel_random_search(self, query):
        telegram_id = query.from_user.id
        cancelled = self.game_logic.cancel_random_wait(telegram_id)
        if cancelled:
            await query.edit_message_text("Поиск остановлен. Возвращаю в меню.")
        else:
            await query.edit_message_text("Поиск уже завершён или игра найдена.")
    ...
    async def create_friend_game(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        msg = await update.message.reply_text(
            self._friend_room_text(game_state),
            parse_mode="HTML",
    ...
    async def edit_friend_categories(
        self, query, context: ContextTypes.DEFAULT_TYPE, data: str
    ):
        game_id = int(data.replace("friend_cats_", "", 1))
        state = self.game_logic.get_game_by_id(game_id)
        if not state or state.host_id != query.from_user.id:
            await query.answer("Только создатель комнаты может менять категории", show_alert=True)
            return
        context.user_data["categories"] = list(state.categories)
        context.user_data["friend_categories_game_id"] = game_id
        await query.edit_message_text(
            "Выбери категории для этой комнаты:",
            reply_markup=categories_keyboard(state.categories),
        )

    async def prompt_friend_rounds(self, query, context: ContextTypes.DEFAULT_TYPE, data: str):
        game_id = int(data.replace("friend_rounds_", "", 1))
        state = self.game_logic.get_game_by_id(game_id)
        if not state or state.host_id != query.from_user.id:
            await query.answer("Только создатель комнаты может менять раунды", show_alert=True)
            return
        await query.edit_message_text(
            f"Раундов в игре сейчас: {state.max_rounds}. Выбери новое значение:",
            reply_markup=friend_rounds_keyboard(state.id, state.max_rounds),
        )

    async def set_friend_rounds(self, query, data: str):
        try:
            _, _, game_id_str, value_str = data.split("_", 3)
            game_id = int(game_id_str)
            value = int(value_str)
        except Exception:
            await query.answer("Некорректные данные раундов", show_alert=True)
            return
        state = self.game_logic.get_game_by_id(game_id)
        if not state or state.host_id != query.from_user.id:
            await query.answer("Только создатель комнаты может менять раунды", show_alert=True)
        ...
        await query.edit_message_text(
            f"Раундов в игре установлено: {value}.",
            reply_markup=friend_rounds_keyboard(state.id, state.max_rounds),
        )
    ...
    async def prompt_friend_players(self, query, data: str):
        game_id = int(data.replace("friend_players_", "", 1))
        state = self.game_logic.get_game_by_id(game_id)
        if not state or state.host_id != query.from_user.id:
            await query.answer("Только создатель комнаты может менять игроков", show_alert=True)
            return
        await query.edit_message_text(
            f"Игроков в комнате может быть от 2 до 10. Сейчас: {state.max_players}.",
            reply_markup=friend_players_keyboard(state.id, state.max_players),
        )

    async def set_friend_players(self, query, data: str):
        try:
            _, _, _, game_id_str, value_str = data.split("_", 4)
            game_id = int(game_id_str)
            value = int(value_str)
        except Exception:
            await query.answer("Некорректные данные игроков", show_alert=True)
            return
        state = self.game_logic.get_game_by_id(game_id)
        if not state or state.host_id != query.from_user.id:
            await query.answer("Только создатель комнаты может менять игроков", show_alert=True)
            return
        state.max_players = max(2, min(value, 10))
        await query.edit_message_text(
            f"Лимит игроков установлен: {state.max_players}.",
            reply_markup=friend_players_keyboard(state.id, state.max_players),
        )

    async def show_friend_room_panel(self, query, data: str):
        game_id = int(data.replace("friend_back_", "", 1))
        state = self.game_logic.get_game_by_id(game_id)
        if not state or state.host_id != query.from_user.id:
            await query.answer("Комната недоступна", show_alert=True)
            return
        await query.edit_message_text(
            self._friend_room_text(state),
            parse_mode="HTML",
            reply_markup=friend_owner_keyboard(state.invite_code, state.id),
        )
    ...
    async def save_categories(self, query, context: ContextTypes.DEFAULT_TYPE):
        ...
        friend_game_id = context.user_data.pop("friend_categories_game_id", None)
        if friend_game_id:
            state = self.game_logic.get_game_by_id(friend_game_id)
            if state and state.host_id == user.id:
    ...
    async def continue_turn(self, query, context: ContextTypes.DEFAULT_TYPE, data: str):
        parts = data.split("_")
        if len(parts) < 2:
            await query.edit_message_text("Ошибка формата данных игры.")
            return
        ...
        user_id = query.from_user.id
        if current_player is not None and user_id != current_player:
            await query.answer("Передать ход может только текущий игрок", show_alert=True)
            return

        await query.edit_message_text("Ход передан следующему игроку.")
        ...
        for uid in game_state.players:
            try:
                if uid == next_player:
                    ...
                    await context.bot.send_message(
                        chat_id=uid,
                        text="Ход переходит другому игроку. Ожидай своей очереди.",
                    )
            except Exception as e:
    ...
    async def start_gender_search_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            ...
            reply_markup=search_preferences_keyboard(
                user_data.get("search_gender", "Любой"),
                self.format_age_label_static(user_data.get("search_age_min"), user_data.get("search_age_max")),
            ),
            parse_mode="HTML",
        )
    ...
    async def start_premium_search(self, query, context: ContextTypes.DEFAULT_TYPE):
        ...
        if game_state is None:
            ...
            categories_text = self._format_categories(categories)
            age_text = self._format_age_range(
                user_data.get("search_age_min"), user_data.get("search_age_max")
            )
            msg = await query.edit_message_text(
                "🔍 Ищем соперника с подходящими параметрами...\n"
                f"Пол поиска: {gender_pref}\n"
                f"Категории: {categories_text}\n"
                f"Возраст: {age_text}\n\n"
                "Как только найдётся второй игрок, игра начнётся автоматически.",
                reply_markup=search_wait_keyboard(),
    ...
    def _search_preferences_text(self, user_data: dict) -> str:
        ...
        if categories_raw:
            try:
                ...
                categories = []
        categories_text = self._format_categories(categories)
        age_text = self._format_age_range(age_min, age_max)
        return (
            "🔍 <b>Поиск по параметрам</b>\n\n"
            f"Пол для поиска: {gender_pref}\n"
            f"Категории: {categories_text}\n"
            f"Возрастной диапазон: {age_text}\n\n"
            "Выбери пол собеседника кнопками ниже или отправь возрастной диапазон собщением."
        )
    ...
    async def prompt_search_age_input(self, query, context: ContextTypes.DEFAULT_TYPE):
        context.user_data["awaiting_search_age_input"] = True
        await query.edit_message_text(
            "Укажи возрастной диапазон для поиска (например 18-30) или одно число.\n"
            "Можно отправить /cancel чтобы выйти без изменений."
        )
    ...
    def _format_premium_until(self, until_value) -> str:
        if not until_value:
            return "неизвестно"
        try:
            if isinstance(until_value, str):
                until_dt = datetime.fromisoformat(until_value)
            elif isinstance(until_value, datetime):
                until_dt = until_value
            else:
                return str(until_value)
            return until_dt.strftime("%Y-%m-%d %H:%M")
        except Exception:
            return str(until_value)
    ...
    def _grant_premium(self, user_id: int, months: int | None = None, days: int | None = None) -> str:
        ...
        return self._format_premium_until(until)
    ...
    async def handle_premium_callback(self, query, context: ContextTypes.DEFAULT_TYPE, data: str):
        if data == "premium_status":
            ...
            if user and user.get("is_premium"):
                until = self._format_premium_until(user.get("premium_until"))
                await query.edit_message_text(
